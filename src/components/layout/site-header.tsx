...
export function SiteHeader() {
  ...
  useEffect(() => {
    ...
    const onPointer = (e: MouseEvent) => {
      if (!navRef.current?.contains(e.target as Node)) setDesktopOpenId(null);
    };
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 bg-white shadow-[0_8px_28px_-20px_rgba(0,0,0,0.28)] motion-safe:transition-transform motion-safe:duration-[420ms] motion-safe:ease-[cubic-bezier(0.22,1,0.36,1)]",
  ...
  if (item.type === "link") {
    return (
      ...
      >
        ...
        <span
          className={cn(
            "absolute inset-x-3.5 bottom-0 h-0.5 rounded-full bg-accent transition-opacity",
    ...
    >
      <div
        className={cn(
          ...
          active || isOpen ? "text-primary" : "text-muted hover:text-primary",
        )}
      >
        <Link
          href={item.href}
        ...
        <span
          className={cn(
            "pointer-events-none absolute inset-x-3.5 bottom-0 h-0.5 rounded-full bg-accent transition-opacity",
            active || isOpen ? "opacity-100" : "opacity-0 group-hover:opacity-70",
          )}
        />
      </div>
      {isOpen ? (
        <div
          ...
          onMouseEnter={() => onOpen(item.id)}
        >
          <div className="min-w-[14rem] overflow-hidden rounded-xl border border-border bg-white py-2 shadow-[0_18px_45px_-20px_rgba(0,0,0,0.3)]">
            {item.links.map((link) => {
              ...
              return (
                <Link
                  ...
                  onClick={onClose}
                  className={cn(
                    "block px-4 py-2.5 text-[13px] font-semibold",
                    linkActive
    ...
    return (
      <Link
        ...
        className={cn(
          ...
          active ? "bg-primary text-white" : "text-primary hover:bg-soft",
        )}
        onClick={onNavigate}
      >
        {item.label}
    ...
    <div className="overflow-hidden rounded-lg border border-border">
      <div className="flex items-stretch">
        <Link
          href={item.href}
          className={cn(
            "flex-1 px-3.5 py-3 text-sm font-semibold",
            active ? "bg-primary text-white" : "bg-white text-primary hover:bg-soft",
          )}
          onClick={onNavigate}
        >
          {item.label}
        ...
        <button
          type="button"
          className={cn(
            "flex w-12 items-center justify-center border-l border-border",
            active ? "bg-primary text-white" : "bg-white text-primary hover:bg-soft",
          )}
          aria-expanded={isOpen}
      ...
      <div
        className={cn(
          "grid motion-safe:transition-[grid-template-rows] motion-safe:duration-[360ms] motion-safe:ease-[cubic-bezier(0.22,1,0.36,1)]",
          isOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]",
        )}
      >
        <ul
          className={cn(
            "overflow-hidden bg-soft/30",
        ...
        >
          {item.links.map((link) => (
            <li key={link.href + link.label}>
              <Link
                href={link.href}
                className="block px-3.5 py-2.5 text-sm font-semibold text-primary hover:bg-white"
                onClick={onNavigate}
              >
                {link.label}
              </Link>
