...
const PILLARS = [
  ...
  { icon: Wrench, title: "Our own workshops", body: "Adaptations are assessed, fitted and serviced by our own engineers, with current training for the products they install." },
...
export default async function AboutPage() {
  ...
  return <>
    ...
    <CatalogIntro eyebrow="Independent specialists. Here for you." title={"Mobility, properly supported."} subtitle="Independent specialists helping people stay mobile — from scooters and wheelchairs to vehicle adaptations fitted by our own engineers." primary={{ href: "/book-a-demo", label: "Book a demonstration" }} secondary={{ href: "/locations", label: "Find your branch" }} image={{ src: "/images/redesign/hero.webp", alt: "Mobility Station adviser helping a customer" }} tone="navy" breadcrumb="About us" />

    <section className="border-b border-border py-14 md:py-20"><div className="container-site grid gap-12 lg:grid-cols-[1.2fr_0.8fr] lg:gap-20"><div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.16em] text-muted">Our approach</p><h2 className="mt-2 text-3xl font-extrabold tracking-tight text-primary md:text-4xl">Built around real life, not pressure.</h2><div className="mt-6 space-y-5 text-base leading-relaxed text-muted md:text-lg"><p>You should be able to try equipment properly before you decide. Visit Heathrow or Ferndown free of charge, or arrange a home visit when that makes more sense.</p><p>From both workshops we support private and Motability customers through assessment, fitting, servicing and long-term support — whether that means a folding chair, a full-size scooter, hand controls or a boot hoist.</p><p>Vehicle adaptations are always checked against your specific car before work is booked. Compatibility matters more than a generic price list.</p></div></div><aside className="rounded-[2rem] bg-primary p-7 text-white md:p-9"><p className="text-xs font-bold uppercase tracking-[0.16em] text-accent-on-dark">Two specialist bases</p><div className="mt-7 space-y-7"><div><h3 className="text-2xl font-extrabold text-white">Heathrow</h3><p className="mt-2 text-sm leading-relaxed text-white/70">West London and the Thames Valley — workshop, demonstrations and local support.</p></div><div className="border-t border-white/15 pt-7"><h3 className="text-2xl font-extrabold text-white">Ferndown</h3><p className="mt-2 text-sm leading-relaxed text-white/70">Dorset and Hampshire — mobility showroom, workshop and South Coast support.</p></div></div><Link href="/locations" className={cn(buttonVariants({ variant: "outline", size: "sm" }), "mt-8 rounded-full border-white/30 bg-white text-primary hover:bg-accent")}><MapPinned className="h-4 w-4" aria-hidden />Opening hours &amp; maps</Link></aside></div></section>

    <section id="training" aria-labelledby="training-title" className="scroll-under-header border-b border-border py-12 md:py-14">
      <div className="container-site grid gap-6 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-muted">Training &amp; accreditation</p>
          <h2 id="training-title" className="mt-2 max-w-sm text-3xl font-extrabold tracking-tight text-primary md:text-4xl">Expertise kept up to date.</h2>
        </div>
        <div className="max-w-3xl space-y-4 text-base leading-relaxed text-muted">
          <p>Our engineers are highly trained. As a Motability-accredited specialist, we make ongoing training part of every engineer’s role, with courses throughout the year, every year.</p>
          <p>We attend manufacturer training in the UK and around the world to gain and maintain accreditation for the adaptations we fit. Manufacturers monitor the training required for their products, and we keep our engineers’ training and authorisations up to date.</p>
          <p className="font-semibold text-primary">No engineer is permitted to work on a vehicle without current training for the adaptations involved.</p>
        </div>
      </div>
    </section>
