...
export default async function BookADemoPage({ searchParams }: { searchParams: Promise<{ product?: string; type?: string }> }) {
  ...
  const defaultEquipmentKind: ScooterWheelchairKind | undefined = defaultCategory !== "scooter_wheelchair" ? undefined
    : category.includes("scooter") ? "scooter"
    : category.includes("powered wheelchair") ? "powered_wheelchair"
    : category.includes("manual wheelchair") ? "manual_wheelchair" : undefined;
  return <>
